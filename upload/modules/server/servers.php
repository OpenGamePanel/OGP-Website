...
<script src="js/jquery.tablesorter.collapsible.js" type="text/javascript"></script>
...
function exec_ogp_module() {
	...
	?><table id="servermonitor" class="tablesorter remote">
		<thead> 
		<tr> 
			<th colspan="4" ><?php print_lang('configured_remote_hosts'); ?></th> 
		</tr> 
		</thead> 
		<tbody> <?php
		...
		if( $host_stat === 1)
        {
			...
			if(preg_match("/Linux/",$os))
			{
				...
				{							
					...
					{
						$remote->sudo_exec("ufw allow ".$server_row['agent_port']); //OGP agent port
						$remote->sudo_exec("ufw allow ".$server_row['ftp_port']); //FTP port
						$remote->sudo_exec("ufw allow 80");//Apache (web server) port
						$remote->sudo_exec("ufw allow 22");//SSH (putty) port
						$remote->sudo_exec("ufw allow 25");//SMTP (mail) port
						$remote->sudo_exec("ufw allow proto tcp to any port 40000:65000");//unknow range of ports needed by the OGP agent
				...
				if($status == "enable")
				{
					$data .= "<b>".get_lang('status')."</b> ".get_lang('on')."<br />
							  <a href='?m=server&amp;rhost_id=".
							  $server_row['remote_server_id']."&amp;ch_ufw_status=disable'>[".get_lang('stop_firewall')."]</a>\n";
				}
				elseif($status == "disable")
				{
					...
					$data .= "<b>".get_lang('status')."</b> ".get_lang('off')."<br />
							  <a href='?m=server&amp;rhost_id=".
							  $server_row['remote_server_id']."&amp;ch_ufw_status=enable'>[".get_lang('start_firewall')."]</a>\n";
				}
				$data .= "<br />";
			}
		}
