...
function exec_ogp_module()
{
	...
	if( is_writable( $templates_folder ) )
	{
		...
		if( isset( $_GET['rserver_id'] ) )
		{
			if( $isAdmin )
			{
				$TS3_list = $db->resultQuery("SELECT * FROM OGP_DB_PREFIXts3_homes WHERE rserver_id='".$_GET['rserver_id']."'");
			}
			else
			{
				$TS3_list = $db->resultQuery("SELECT * FROM OGP_DB_PREFIXts3_homes WHERE user_id='".$_SESSION['user_id']."' AND rserver_id='".$_GET['rserver_id']."'");
			}
			
			if( isset( $_POST['vserver_id'] ) )
			{
				$remote_server = $db->getRemoteServer($_POST['rserver_id']);
				$remote_key = $remote_server['encryption_key'];
				$password = decrypt($_POST['pwd'],$remote_key);
				$_SESSION['ts3_ip'] = $_POST['ip'];
				$_SESSION['ts3_pwd'] = $password;
				$_SESSION['ts3_vserver_id'] = $_POST['vserver_id'];
			}
			...
			if( !empty( $TS3_list ) )
			{
				...
				else
				{
					...
					foreach( $TS3_list as $TS3 )
					{
						$remote_server = $db->getRemoteServer($TS3['rserver_id']);
						$remote_key = $remote_server['encryption_key'];
						$password = encrypt($TS3['pwd'],$remote_key);
						$counter++;
						echo "<td><form action='' method='POST'>
							  <input type='hidden' name='rserver_id' value='". $TS3['rserver_id'] . "' />
							  <input type='hidden' name='ip' value='". $TS3['ip'] . "' />
			...
			{
				echo "There Are No TS3 Servers Available.";
				return;
			}
		}
		if( !isset( $_SESSION['ts3_ip'] ) ) return;
