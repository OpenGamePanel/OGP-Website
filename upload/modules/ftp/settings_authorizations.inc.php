...
$ips_query = mysql_query("SELECT agent_ip,ftp_ip FROM " . $table_prefix . "remote_servers");

$id=1;
while($ips_array=mysql_fetch_array($ips_query,MYSQL_ASSOC)) 
{
	if(empty($ips_array['ftp_ip']))
	{
		$net2ftp_settings["allowed_ftpservers"][$id] = $ips_array['agent_ip'];
		$id++;
	}
	else
	{
		$net2ftp_settings["allowed_ftpservers"][$id] = $ips_array['ftp_ip'];
		$id++;
	}
} 
