...
class OGPView {
    ...
    function printView($cleared = false) {
        ...
        if ( is_object($db) && array_key_exists( "OGPDatabase", class_parents($db) ) ) {
            $footer .= "<div class=\"footer center\">".@$panel_settings['theme']." ".get_lang('theme')."<br />".get_lang('copyright')." &copy; <a href=\"http://www.opengamepanel.org\">Open Game Panel</a> 2013 - ".get_lang('all_rights_reserved').".<br />v".@$panel_settings['ogp_version']." - ".
                $db->getNbOfQueries()." ".get_lang('queries_executed')."</div>";
        }
        else
        {
            $footer .= "<div class='footer center'>".get_lang('copyright')." &copy; <a href=\"http://www.opengamepanel.org\">Open Game Panel</a> 2013 - ".get_lang('all_rights_reserved').".</div>";
        }
