...
//------------------------------------------------------------------------------------------------------------+
	...
	function lgsl_port_conversion($type, $c_port, $q_port, $s_port)
	{
		...
		{
			case "7dtd"				: $c_to_q = 1;		$c_def = 25000;	$q_def = 8081;	$c_to_s = 0;	break;
...
//------------------------------------------------------------------------------------------------------------+  
	function lgsl_query_40(&$server, &$lgsl_need, &$lgsl_fp)
	{
		...
		$home_info = $server_home = $db->getGameHomeByIP($ip, $port);
		$remote = new OGPRemoteLibrary($home_info['agent_ip'],$home_info['agent_port'],$home_info['encryption_key']);
		$return = $remote->exec('exec 3<>/dev/tcp/'.$server['b']['ip'].'/'.$server['b']['q_port'].' && echo -en "lp\\nexit\\n" >&3 && cat <&3');

		if(preg_match("/Connected with 7DTD server/",$return))
		{
			preg_match_all("(World:(.*)\n)siU", $return, $map);
			$server['s']['map'] = trim($map[1][0]);
			preg_match_all("(Game name:(.*)\n)siU", $return, $name);
			$server['s']['name'] = trim($name[1][0]);
			preg_match_all("(Max players:(.*)\n)siU", $return, $maxplayers);
			$server['s']['playersmax'] = trim($maxplayers[1][0]);
			preg_match_all("(Total of (.*) in the game)siU", $return, $players);
			$server['s']['players'] = trim($players[1][0]);
			preg_match_all("(to end session\.(.*)Total of)siU", $return, $pl);
			$playerlist = trim($pl[1][0]);
			$users = explode("\n",$playerlist);
			...
			if( $server['s']['players'] > 0)
			{
				...
				foreach($users as $user)
				{
					...
					$server['p'][$i]['name']  = trim(str_replace('(EntityPlayer)','',$user_info[1]));
					$user_health = explode("=",$user_info[10]);
					$server['p'][$i]['score'] = $user_health[1];
					$i++;
				}
			}
			return TRUE;
