...
try
{
	...
	if($ts3_ServerInstance->isOnline() == TRUE)
	{
		...
		if(isset($_POST['new_ts3_port']) AND $server_home['home_id'] == $_POST['home_id'] )
		{
			...
			{		
				$remote->sudo_exec("ufw allow ".$new_port);
			}
			if ($AddVirtual === TRUE)
			{
				...
				catch(Exception $e)
				{
					...
					if (isset($server_home['ufw_status']) and $server_home['ufw_status'] == "enable")
					{		
						$remote->sudo_exec("ufw deny ".$new_port);
					}
				}
		...
		{
			$startup_file_exists = $remote->rfile_exists( "startups/".$server_home['ip']."-".$_POST['del_ts3_port'] ) === 1;
		...
		$add_remove_virtual = "<table class='center'>
							   <tr><th>Remove This<br>Virtual Server</th></tr>
							   <tr><td>
							   ...
							   <table class='center'>
							   <tr><th>Add Virtual Server</th></tr>
							   <tr><td colspan=2>
							   <form action='' method='POST'>
		...
		if( $TS3Admin_installed )
		{
			...
			$add_remove_virtual .= "<tr><td>Assign Virtual<br>Server To User</td><td>
									<form action='' method='POST'>
