...
class GameQ
{
	...
	public function requestData()
	{
		...
		$data = array();
		global $settings;
		if (isset($settings['remote_query']) and $settings['remote_query'] == 1)
		{
			global $db;
			foreach($this->servers AS $server_id => $instance)
			{
				$home_info = $db->getGameHomeByIP($instance->ip(), $instance->port_client());
				if(is_array($home_info) && !empty($home_info))
				{
					require_once('includes/lib_remote.php');
					$remote = new OGPRemoteLibrary($home_info['agent_ip'], $home_info['agent_port'], $home_info['encryption_key'], $home_info['timeout']);
					$return = $remote->remote_query('gameq', $instance->name(), $instance->ip(), $instance->port_client(), $instance->port(), $instance->port_client());
					if($return == NULL) continue;
					if (preg_match("/_SGAMEQF_(.*)_SGAMEQF_/U", $return, $match))
						$data[$server_id] = unserialize($match[1]);
					else
						continue;
				}
			}
