...
define('OGP_LANG_custom_github_update_branch_name', "Имя ветки GitHub");
define('OGP_LANG_custom_github_update_branch_name_info', "Введите имя ветки, которую вы хотите использовать для обновления OGP. Это должно быть изменено только разработчиками, которые хотят использовать свои собственные репозитории для разработки, а не проверять код с возможными ошибками в основной ветке.&nbsp; Оставьте это поле пустым, чтобы по умолчанию было \"master\"");
...
define('OGP_LANG_use_authorized_hosts', "Ограничение API для определенных авторизованных хостов");
define('OGP_LANG_use_authorized_hosts_info', "Включите этот параметр, чтобы разрешать вызовы API только с предварительно определенных и утвержденных IP-адресов.&nbsp; Одобренные адреса можно указать на этой странице после включения этой настройки.&nbsp; Если этот параметр отключен, пользователь, использующий действительный ключ, будет иметь доступ к API с любого IP-адреса.&nbsp; Пользователи, использующие действительный ключ, смогут использовать API для управления любым игровым сервером, на управление которым у них есть права.");
define('OGP_LANG_allow_setting_cpu_affinity', "Разрешить назначение ядер ЦП для Игровых Серверов");
define('OGP_LANG_allow_setting_cpu_affinity_info', "Если этот параметр включен, администратору, создающему игровой сервер, будут показаны параметры привязки ЦП (назначения ядра) для игрового сервера.");
...
define('OGP_LANG_reset_game_server_order', "Сбросить порядок Игровых Серверов");
define('OGP_LANG_reset_game_server_order_info', "Сбрасывает порядок игровых серверов до значения по умолчанию с использованием идентификатора сервера ID.");
define('OGP_LANG_regex_invalid_file_name_chars', "Недопустимые символы в имени файла Regex");
define('OGP_LANG_regex_invalid_file_name_chars_info', "Измените этот шаблон регулярного выражения, если вы хотите разрешить другой набор символов в именах файлов.");
define('OGP_LANG_login_ban_time', "Время блокировки при неудачном Входе в Систему (в секундах)");
define('OGP_LANG_login_ban_time_info', "Время в секундах, в течение которого IP-адресу запрещается пытаться войти в панель после определенного количества неудачных попыток входа.");
?>
