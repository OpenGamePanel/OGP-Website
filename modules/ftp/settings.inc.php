...
if ( is_readable("../../includes/config.inc.php") ) require_once("../../includes/config.inc.php");

$conect_db = mysql_connect($db_host, $db_user, $db_pass, $db_name);
$select_db = mysql_select_db($db_name);

$settings_query = mysql_query("SELECT * FROM " . $table_prefix . "settings");
$panel_email_address = "";
while($setting = mysql_fetch_array($settings_query,MYSQL_ASSOC)) 
{
	if($setting['setting'] == 'panel_email_address')
		$panel_email_address = $setting['value'];
	if($setting['setting'] == 'panel_language')
		$panel_language = $setting['value'];
}
...
session_start();
$net2ftp_settings["default_language"] = (isset($_SESSION['users_lang']) and $_SESSION['users_lang'] != "-") ? lang_str($_SESSION['users_lang']) : 
										( (isset($panel_language) && $panel_language != "") ? lang_str($panel_language): "en");
